...
def list_vessels(
    ...
    vessel_type: Optional[str] = Query(None, description="Filter by vessel category"),
...
@router.get("/{vessel_id}/track", summary="Get chronological track telemetry and anomaly paths for a vessel")
def get_vessel_track(vessel_id: str):
    track_data = ais_service.get_vessel_track(vessel_id)
    if not track_data:
        raise HTTPException(status_code=404, detail=f"Track for vessel '{vessel_id}' not found.")
    return track_data
